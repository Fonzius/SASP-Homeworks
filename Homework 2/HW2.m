...
y1_stft = y1_stft(:, 1:padded_length/2);
y2_stft = y2_stft(:, 1:padded_length/2);
...
f1_reshape = ifft(f1_stft_flip, padded_length, 2);
f2_reshape = ifft(f2_stft_flip, padded_length, 2);
f3_reshape = ifft(f3_stft_flip, padded_length, 2);
...
%% Output
audiowrite("f1.wav",real(f1), fs);
audiowrite("f2.wav",real(f2), fs);
audiowrite("f3.wav",real(f3), fs);
...
subplot(2,3,4);
surf(t,f,20*log10(abs(f1_stft')), EdgeColor="none");
...
subplot(2,3,5);
surf(t,f,20*log10(abs(f2_stft')), EdgeColor="none");
...
subplot(2,3,6);
surf(t,f,20*log10(abs(f3_stft')), EdgeColor="none");
